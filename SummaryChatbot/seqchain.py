...
model = OllamaLLM(model='llama3.1:8b',num_predict=150)

instructions = pydanticparser.get_format_instructions()

prompt1 = PromptTemplate(
    template = 'Make a summary of the following {content} given by user.',
    input_variables=['content'],
)

prompt2 = PromptTemplate(
    template="Make a questions and answer from this summary {summary}.",
    input_variables=['summary'],
)
chain1 = prompt1 | model | strparser | prompt2 | model | strparser
...
res = chain1.invoke({'content':text})
print(res, 'result')
